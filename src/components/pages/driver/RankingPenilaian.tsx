  ...
  useEffect(() => {
    fetch(`${apiBase}/api/siklus`, { headers: authHeader() })
      ...
      .then(d => {
        ...
        // Auto-select siklus yang berjalan
        const berjalan = available.find((s: Siklus) => s.status_display === 'berjalan')
        if (berjalan) setSelectedSiklus(berjalan.siklus_id)
        else if (available.length > 0) setSelectedSiklus(available[0].siklus_id)
      })
      .catch(() => {})
  }, [])
