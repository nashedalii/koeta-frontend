...
export default function KelolaKoridor() {
  ...
  return (
    <div>
      {/* ── Stat Cards ── */}
      {!loading && (() => {
        const totalKoridor = koridors.length
        const jumlahKoridor = koridors.filter(k => k.tipe === 'koridor').length
        const jumlahFeeder  = koridors.filter(k => k.tipe === 'feeder').length

        const baseCards = [
          { label: 'Total Koridor', value: totalKoridor, color: '#0369a1', bg: '#e0f2fe' },
          { label: 'Koridor', value: jumlahKoridor, color: '#2563eb', bg: '#dbeafe' },
          { label: 'Feeder', value: jumlahFeeder, color: '#d97706', bg: '#fef3c7' },
        ]
        ...
        if (isSuperAdmin) {
          ...
          koridors.forEach(k => {
            if (k.armada_id && k.nama_armada) {
              const key = String(k.armada_id)
              if (!armadaCount[key]) armadaCount[key] = { nama: k.nama_armada, count: 0 }
